...
!
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
       namelist /PelChem_parameters/ p_sN4N3, p_q10N4N3, p_q10R6N5,     &
     &   p_clN6r, p_sN3O4n, p_rPAo, p_sR6N5, p_rOS, p_clO2o
#ifdef INCLUDE_PELFE
       namelist /PelChem_parameters_iron/ p_q10R6N7, p_sR6N7,           &
     &   p_sR1N7, p_scavN7f, p_N7fsol
#endif
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
